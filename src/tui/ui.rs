...
impl TuiConfig {
    ...
    /// Create a configuration with initial query
    pub fn with_initial_query(query: String) -> Self {
        Self {
            fullscreen: true,
            height: None,
            height_percentage: None,
...
async fn run_async_interactive_tui(
    ...
    if let Some(initial_query) = &config.initial_query {
        fuzzy_finder.set_query(initial_query.clone()).await;
    }
...
mod tests {
    ...
    #[test]
    fn test_tui_config_with_initial_query() {
        let config = TuiConfig::with_initial_query("test query".to_string());
        assert!(config.fullscreen);
        assert!(config.height.is_none());
        assert!(config.height_percentage.is_none());
        assert!(config.show_help_text);
        assert_eq!(config.initial_query, Some("test query".to_string()));
    }
