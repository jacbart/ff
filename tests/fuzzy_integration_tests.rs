use ff::fuzzy::{FuzzyFinder, fuzzy_match};
...
fn test_update_filter_empty_query() {
    ...
    assert_eq!(finder.filtered_items, vec!["apple".to_string(), "banana".to_string()]);
    assert_eq!(finder.cursor_position, 0);
}

#[test]
fn test_update_filter_with_query() {
    let mut finder = FuzzyFinder::new(vec!["apple".to_string(), "banana".to_string(), "cherry".to_string()], false);
...
fn test_get_selected_items_multi_mode() {
    let mut finder = FuzzyFinder::new(vec!["apple".to_string(), "banana".to_string(), "cherry".to_string()], true);
    finder.cursor_position = 0;
    finder.toggle_selection();
...
fn test_large_dataset_parallel_filtering() {
    ...
    assert!(finder.filtered_items.len() > 0);
}

#[test]
fn test_special_characters_in_query() {
    let items = vec!["test-item".to_string(), "test_item".to_string(), "testitem".to_string()];
    let mut finder = FuzzyFinder::new(items, false);
